    ...
    with open('neo_downstream_samplesheet.csv', 'w', newline='') as outfile:
        ...
        for row in rows:
            if row['sample_id'].lower() != 'germline':
                ...
                variants_expression = f"${base_dir}/results/pdmr/{patient_id}/sarek/vcf_expression_annotation/clean_vcf/{patient_id}_{sample_id}_neoantigen.csv"
                binding_predictions = f"${base_dir}/results/pdmr/{patient_id}/epitopeprediction/predictions/{patient_id}_{sample_id}.tsv"
